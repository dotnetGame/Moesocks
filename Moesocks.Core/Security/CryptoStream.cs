...
namespace Moesocks.Security
{
    public class CryptoStream : Stream, IDisposable
    {
        ...
        public CryptoStream(Stream stream, ICryptoTransform transform, CryptoStreamMode mode, bool leaveOpen)
        {
            ...
            switch (_transformMode)
            {
                case CryptoStreamMode.Read:
                    if (!(_stream.CanRead)) throw new ArgumentException(nameof(stream));
                    _canRead = true;
                    break;
                case CryptoStreamMode.Write:
                    if (!(_stream.CanWrite)) throw new ArgumentException(nameof(stream));
                    _canWrite = true;
                    break;
                default:
        ...
        public void FlushFinalBlock()
        {
            ...
            if (_canWrite)
                _stream.Write(finalBytes, 0, _inputBufferIndex);
        ...
        public Task<int> ReadSomeAsync(byte[] buffer, int offset, int count)
        {
            CheckReadArguments(buffer, offset, count);
            return ReadAsyncCore(buffer, offset, count, default(CancellationToken), true, false);
        }

        private async Task<int> ReadAsyncCore(byte[] buffer, int offset, int count, CancellationToken cancellationToken, bool useAsync, bool exact = true)
        {
            ...
            if (bytesToDeliver > _outputBlockSize)
            {
                if (_transform.CanTransformMultipleBlocks)
                {
                    ...
                    var toRead = numWholeBlocksInBytes - _inputBufferIndex;
                    amountRead += useAsync ?
                        await _stream.ReadAsync(tempInputBuffer, _inputBufferIndex, toRead, cancellationToken) :
                        _stream.Read(tempInputBuffer, _inputBufferIndex, toRead);
            ...
            {
                if(!exact && _inputBufferIndex < _inputBlockSize)
            ...
            Array.Clear(_inputBuffer, _inputBufferIndex, _inputBuffer.Length - _inputBufferIndex);
            byte[] finalBytes = _transform.TransformFinalBlock(_inputBuffer, 0, _inputBuffer.Length);
            ...
            _outputBufferIndex = _inputBufferIndex;
